  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSuccessMsg(
          `Berat preliminary ${numericWeight} kg berhasil dicatat. Pihak laundry akan memverifikasi berat ini saat pakaian diterima.`
        );
        setTimeout(() => {
          onSuccess();
        }, 1200);
      } else {
        setErrorMsg(data.message || 'Gagal menyimpan berat preliminary.');
      }
    } catch (err: any) {
      ...
      <Card variant="white" className="w-full max-w-md p-5 space-y-4 relative shadow-2xl rounded-2xl animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm text-slate-900">Penimbangan Awal Kurir</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Preliminary Info Notice */}
          <div className="p-3 bg-blue-50/80 rounded-xl border border-blue-200/70 text-xs text-blue-800 leading-relaxed">
            <p className="font-medium">
              ℹ️ <strong>Catatan Kurir:</strong> Berat yang Anda masukkan bersifat preliminary (sementara). Pihak Laundry akan melakukan verifikasi fisik saat menerima pakaian sebelum total harga difinalisasi.
            </p>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-slate-700">
              Berat Hasil Penimbangan Kurir ({unitName}):
            </label>
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {isSubmitting ? 'MENYIMPAN...' : 'SIMPAN BERAT KURIR'}
            </Button>
          </div>
        </form>
