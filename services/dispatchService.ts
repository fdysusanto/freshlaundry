...
export const dispatchService = {
   ...
   */
  async isCourierBusyAsync(courierId: string): Promise<boolean> {
    if (!isSupabaseConfigured || !supabase) {
  ...
  async findEligibleCouriersAsync(
    ...
    for (const c of rawCouriers) {
      ...
      const isBusy = await this.isCourierBusyAsync(c.id);
      if (isBusy) continue;
  ...
  async dispatchOrderAsync(
    ...
    const existingOrder = await orderService.getOrderByIdAsync(orderId);
    ...
    const currentStatus = await this.getDispatchStatusAsync(orderId);
    ...
    if (isSupabaseConfigured && supabase) {
      const { data: prevAssignments } = await (supabase.from('courier_assignments') as any)
        .select('courier_id')
        .eq('order_id', orderId);
      previousCourierIds = (prevAssignments || []).map((x: any) => x.courier_id);
    }

    const candidates = await this.findEligibleCouriersAsync(orderId, assignmentType, radiusKm, previousCourierIds);
    if (candidates.length === 0 && batchNumber < DISPATCH_CONFIG.MAX_BATCHES) {
      ...
      const escalatedCandidates = await this.findEligibleCouriersAsync(orderId, assignmentType, escalatedRadius, previousCourierIds);
    ...
    // Insert dispatch_batches row
    const { data: insertedBatch, error: batchErr } = await (supabase.from('dispatch_batches') as any)
      .insert({
        order_id: orderId,
    ...
    for (const candidate of candidates) {
      await (supabase.from('courier_assignments') as any).insert({
        order_id: orderId,
   ...
   */
  async getDispatchStatusAsync(orderId: string): Promise<DispatchStatusResult> {
    if (!isSupabaseConfigured || !supabase) {
    ...
    const { data: latestBatch } = await (supabase.from('dispatch_batches') as any)
   ...
   */
  async processExpiredDispatchBatchesAsync(): Promise<void> {
    const now = new Date().toISOString();
    ...
    const { data: expiredBatches } = await (supabase.from('dispatch_batches') as any)
      .select('*, orders(*)')
      .eq('status', 'active')
      .lte('expires_at', now);
    ...
    for (const batch of expiredBatches) {
      // Mark batch expired
      await (supabase.from('dispatch_batches') as any)
        .update({ status: 'expired' })
        .eq('id', batch.id);

      // Mark unaccepted offers expired
      await (supabase.from('courier_assignments') as any)
        .update({ status: 'expired' })
      ...
      if (order && order.payment_status === 'paid') {
        ...
        if (canTriggerNext && batch.batch_number < DISPATCH_CONFIG.MAX_BATCHES) {
          try {
            await this.dispatchOrderAsync(order.id, batch.assignment_type, order.laundry_id);
          } catch (err: any) {
   ...
   */
  async retryDispatchAsync(orderId: string, actorUserId: string): Promise<DispatchStatusResult> {
    const { orderService } = await import('./orderService');
    ...
    return this.dispatchOrderAsync(orderId, assignmentType, actorUserId);
  },
};
