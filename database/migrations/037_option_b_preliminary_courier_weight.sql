-- Migration: 037_option_b_preliminary_courier_weight.sql
-- Description: Implements Option B Weight Flow.
-- 1. Adds courier_weight_kg, weight_finalized_at, weight_finalized_by to orders.
-- 2. Adds save_courier_preliminary_weight_atomic RPC for couriers (no price recalculation, no payment trigger).
-- 3. Adds finalize_laundry_weight_atomic RPC for laundry staff/owners (recalculates subtotal and triggers price delta).

BEGIN;

-- 1. Additive Column Migrations on orders table
ALTER TABLE public.orders 
ADD COLUMN IF NOT EXISTS courier_weight_kg NUMERIC NULL,
ADD COLUMN IF NOT EXISTS weight_finalized_at TIMESTAMPTZ NULL,
ADD COLUMN IF NOT EXISTS weight_finalized_by UUID NULL REFERENCES public.profiles(id);

COMMENT ON COLUMN public.orders.courier_weight_kg IS 'Preliminary weight weighed by courier upon pickup (informational only, no price recalculation)';
COMMENT ON COLUMN public.orders.weight_finalized_at IS 'Timestamp when laundry verified and finalized actual weight';
COMMENT ON COLUMN public.orders.weight_finalized_by IS 'User ID of laundry owner/staff who finalized the weight';

-- 2. Courier Preliminary Weigh Atomic RPC
...
BEGIN
  ...
  -- 4. State Validation (Allowed in 'pending', 'assigned', 'picked_up')
  IF v_order.status NOT IN ('pending', 'assigned', 'picked_up') THEN
    RAISE EXCEPTION 'Penimbangan Ditolak: Status pesanan saat ini (%) tidak memungkinkan penimbangan preliminary.', v_order.status;
  END IF;
...
-- 3. Laundry Finalize Weight Atomic RPC
...
BEGIN
  ...
  -- 4. Immutability Lock: Block modification if Payment Adjustment is already PAID (unless Admin)
  ...
  -- 5. Atomic Order Items Update & Price Recalculation (with Minimum Charge Floor)
  ...
  -- 6. Scoped Local Transaction Bypass for Column Tampering Trigger
