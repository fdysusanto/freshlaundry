...
-- Migration: 041_fix_approve_partner_application_laundries_schema.sql
-- Description: Fixes approve_partner_application RPC by removing invalid 'city'
--              and 'district' destination columns from INSERT INTO public.laundries
--              and restoring correct regional mapping (city_name, district_name).
--              Preserves Phase 7 (min_weight) and Phase 8 (estimated_hours) features.
...
BEGIN
  ...
  -- F. Step 1 Provisioning: Insert ke public.laundries dengan schema wilayah yang valid
