...
-- Migration: 044_fix_approve_partner_application_schema_compatibility.sql
-- Description: Replaces approve_partner_application RPC to ensure 100% schema
--              compatibility with actual production Supabase tables:
--              1. Removes references to non-existent partner_applications.approved_laundry_id.
--              2. Removes references to non-existent profiles.updated_at.
--              3. Updates partner_applications with valid columns (status, reviewed_by, reviewed_at, updated_at).
--              4. Ensures collision-safe laundry code generation loop.
--              5. Preserves Phase 7 (min_weight), Phase 8 (estimated_hours), Phase 9 (laundries location schema),
--                 and Phase 10 Revised (multi-service enum architecture).
...
BEGIN
  ...
  -- D. Check Idempotensi: Jika sudah approved (DO NOT reference approved_laundry_id)
