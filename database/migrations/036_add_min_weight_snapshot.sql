...
-- Migration: 036_add_min_weight_snapshot.sql
-- Description: Adds min_weight_snapshot column to public.order_items table.
--              Snapshots minimum charge (min_weight) at checkout time to guarantee
--              100% historical pricing contract immutability.
--              Includes hybrid backfill for legacy orders and updates
--              update_order_actual_weight_atomic RPC to prioritize min_weight_snapshot.
...
-- 1. Add min_weight_snapshot column to public.order_items table
ALTER TABLE public.order_items
ADD COLUMN IF NOT EXISTS min_weight_snapshot NUMERIC(6, 2);

-- 2. Hybrid Data Backfill for Legacy Order Items
-- Case A: Unambiguous mathematical reconstruction for unweighed orders where quantity < subtotal / price_snapshot
-- Case B & Fallback: Backfill from catalog services.min_weight or default 1
UPDATE public.order_items oi
SET min_weight_snapshot = CASE
  WHEN oi.actual_weight IS NULL AND oi.price_snapshot > 0 AND oi.quantity < ROUND(oi.subtotal / NULLIF(oi.price_snapshot, 0), 2)
    THEN ROUND(oi.subtotal / NULLIF(oi.price_snapshot, 0), 2)
  ELSE COALESCE(s.min_weight, 1)
END
FROM public.services s
WHERE oi.service_id = s.id AND oi.min_weight_snapshot IS NULL;

-- 3. Failsafe Backfill for any orphan order_items (where service_id is deleted/unmapped)
UPDATE public.order_items
SET min_weight_snapshot = 1
WHERE min_weight_snapshot IS NULL;

-- 4. Add Check Constraint ensuring valid min_weight_snapshot (>= 1)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'chk_order_items_min_weight_snapshot'
  ) THEN
    ALTER TABLE public.order_items
    ADD CONSTRAINT chk_order_items_min_weight_snapshot
    CHECK (min_weight_snapshot IS NULL OR min_weight_snapshot >= 1);
  END IF;
END $$;

-- 5. Update update_order_actual_weight_atomic RPC to enforce min_weight_snapshot floor
...
BEGIN
  ...
  -- 8. Atomic Order Items Update & Price Recalculation (Database Source of Truth with Minimum Charge Floor)
  -- Priority Order for Minimum Charge Floor:
  -- 1. oi.min_weight_snapshot (Immutable Historical Contract)
  -- 2. services.min_weight (Catalog fallback for legacy active orders)
  -- 3. 1.0 (Failsafe minimum)
  UPDATE public.order_items oi
