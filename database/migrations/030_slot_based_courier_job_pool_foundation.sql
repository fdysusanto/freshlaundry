...
-- Migration: 030_slot_based_courier_job_pool_foundation.sql
-- Description: Foundation for Slot-Based Courier Job Pool Architecture.
--              1. Drops 1-order constraint uq_courier_single_active_order.
--              2. Adds Atomic PostgreSQL RPC claim_slot_job_batch_atomic() with
--                 FOR UPDATE SKIP LOCKED, 15-minute claim window validation,
--                 max 5 orders capacity rule, and zero PII leakage.
...
-- 1. DROP LEGACY 1-ORDER ACTIVE CONSTRAINT
DROP INDEX IF EXISTS public.uq_courier_single_active_order;

-- 2. CREATE HELPER FUNCTION FOR WIB TIMESTAMP PARSING
CREATE OR REPLACE FUNCTION public.parse_wib_slot_start(
  p_date TEXT,
  p_time_slot TEXT
...
BEGIN
  IF p_time_slot IS NOT NULL THEN
    v_slot_match := regexp_matches(p_time_slot, '(\d{1,2}):(\d{2})');
    IF v_slot_match IS NOT NULL AND array_length(v_slot_match, 1) >= 2 THEN
      v_hour := v_slot_match[1]::INTEGER;
      v_minute := v_slot_match[2]::INTEGER;
    END IF;
  END IF;

  RETURN (p_date || 'T' || lpad(v_hour::TEXT, 2, '0') || ':' || lpad(v_minute::TEXT, 2, '0') || ':00.000+07:00')::TIMESTAMPTZ;
EXCEPTION
  WHEN OTHERS THEN
    RETURN NULL;
END;
...
-- 3. ATOMIC SLOT CLAIM RPC FUNCTION
...
BEGIN
  ...
  ELSE
    FOR v_rec IN
      SELECT id FROM public.orders
      WHERE delivery_date = p_job_date
        AND delivery_time_slot = p_time_slot
        AND payment_status = 'paid'
