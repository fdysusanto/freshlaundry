...
-- Migration: 047_fix_delivery_courier_ownership.sql
-- Description: Resets courier_id to NULL when orders reach ready_for_delivery
--              so orders enter the Delivery Job Pool cleanly.
--              Ensures claim_slot_job_batch_atomic creates explicit delivery assignment records.
...
-- Set transaction context flag so database trigger prevent_order_column_tampering() 
-- allows system migration to update orders.courier_id to NULL
SELECT set_config('app.courier_assignment', 'true', true);

-- 1. Fix Existing Data: Reset courier_id for unassigned ready_for_delivery orders
...
-- 2. Update Atomic Slot Claim RPC to record explicit delivery courier assignment
...
BEGIN
  ...
  IF p_job_type = 'pickup' THEN
    ...
    LOOP
      ...
      INSERT INTO public.courier_assignments (order_id, courier_id, assignment_type, status, responded_at)
      VALUES (v_rec.id, p_courier_id, 'pickup', 'accepted', NOW())
  ...
  ELSE
    ...
    LOOP
      ...
      INSERT INTO public.courier_assignments (order_id, courier_id, assignment_type, status, responded_at)
      VALUES (v_rec.id, p_courier_id, 'delivery', 'accepted', NOW())
      ON CONFLICT DO NOTHING;
