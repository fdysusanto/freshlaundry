...
-- Migration: 035_fix_actual_weight_minimum_charge_rpc.sql
-- Description: Fixes update_order_actual_weight_atomic RPC to enforce service
--              minimum_charge (min_weight) floor when calculating subtotal & delta.
--              Physical actual_weight is stored as weighed (e.g. 3 kg), but
--              subtotal is calculated as GREATEST(actual_weight, min_weight) * price.
...
BEGIN
  ...
  -- Note: Physical actual_weight is stored as p_final_weight_kg, but subtotal is calculated as
  --       GREATEST(p_final_weight_kg, service.min_weight) * price_snapshot
