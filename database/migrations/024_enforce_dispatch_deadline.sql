-- Migration: 024_enforce_dispatch_deadline.sql
-- Description: Enforce server-side lazy expiration for dispatch batches and time slots.
...
BEGIN
  ...
  IF v_batch_id IS NOT NULL THEN
    ...
    -- LAZY EXPIRATION: Enforce expires_at strictly on the database side
    IF FOUND AND v_batch_expires_at <= NOW() THEN
      RAISE EXCEPTION 'Penawaran kurir sudah kedaluwarsa.';
    END IF;
  END IF;

  -- 5. Lock and validate order
  SELECT status, payment_status, pickup_date, pickup_time_slot, delivery_date, delivery_time_slot
  INTO v_order_status, v_payment_status, v_pickup_date, v_pickup_time_slot, v_delivery_date, v_delivery_time_slot
  FROM public.orders
  WHERE id = v_order_id
  FOR UPDATE;
  ...
  IF v_assignment_type = 'pickup' THEN
    ...
    -- BUSINESS RULE: Enforce pickup time slot expiration
    IF v_pickup_date IS NOT NULL AND v_pickup_time_slot IS NOT NULL AND v_pickup_time_slot != '' THEN
      ...
      IF v_slot_end_time != '' THEN
        ...
        IF NOW() >= v_slot_end_tz THEN
          RAISE EXCEPTION 'Batas waktu pickup sudah terlewat.';
        END IF;
      END IF;
    END IF;
  ...
  ELSIF v_assignment_type = 'delivery' THEN
    ...
    -- BUSINESS RULE: Enforce delivery time slot expiration
